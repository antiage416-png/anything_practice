...
**한국은행 기준금리와의 관계 — 한미 금리 역전**

한국은행은 미국 금리를 그대로 따라가지는 않지만, 한미 금리차가 과도하게 벌어지면 아래와 같은 압력을 받습니다.

```mermaid
flowchart LR
    A["미국 기준금리 > 한국 기준금리\n(한미 금리 역전)"] --> B["원화 자산 매력 저하"]
    B --> C["외국인 자금 유출 우려"]
    B --> D["원화 약세 압력"]
    C --> E["한국은행도 인상 압력 직면"]
    D --> E
...
> ⚠️ **주의**: 한미 금리차가 벌어져도 한국은행은 내수 경기, 가계부채, 환율 안정 등을 종합적으로 고려해 독자적으로 결정합니다. "미국이 올리면 한국도 무조건 올린다"는 단순 도식은 아닙니다.

> 📺 [🎬 FOMC 기준금리 결정 뜻](https://www.youtube.com/results?search_query=FOMC+기준금리+결정+한국어+설명)

#### 🔗 Python 소스 연계

미국 기준금리(연방기금금리)와 국채금리는 웹앱 **거시경제현황 1 (실시간)** 탭에서 `^TNX`(10년물), `^IRX`(3개월물) 등으로 조회할 수 있습니다.

```python
import yfinance as yf

# 미국 국채 금리 프록시 (FOMC 정책 기대가 선반영되는 지표)
us10y = yf.download("^TNX", period="1y", auto_adjust=True)["Close"]  # 10년물
us3m  = yf.download("^IRX", period="1y", auto_adjust=True)["Close"]  # 3개월물(정책금리에 가장 민감)

print(f"10년물 최근값: {us10y.iloc[-1]:.2f}")
print(f"3개월물 최근값(정책금리 근접): {us3m.iloc[-1]:.2f}")
```

| FOMC 일정 확인용 사이트 | URL | 비고 |
|---|---|---|
| 연준(Fed) 공식 캘린더 | <https://www.federalreserve.gov/monetarypolicy/fomccalendars.htm> | 회의 일정·성명서·점도표 원문 |
| FRED 연방기금금리 | <https://fred.stlouisfed.org/series/FEDFUNDS> | 과거 금리 결정 이력 시계열 |
